  ...
  return (
    ...
    <div className="w-full max-w-lg mx-auto px-1 pb-1 select-none">
      ...
      <div className="flex gap-2 px-4">
        <button
          onClick={onEnter}
          // UPDATED: Reduced height from h-14 to h-9 (~35% smaller)
          className="flex-1 h-9 bg-[#565758] rounded-lg font-bold text-xs sm:text-sm text-white uppercase tracking-wider active:bg-[#404040] transition-colors"
        >
          ENTER
        </button>
        <button
          onClick={onDelete}
          // UPDATED: Reduced height from h-14 to h-9
          className="flex-1 h-9 bg-[#565758] rounded-lg font-bold text-xs sm:text-sm text-white uppercase tracking-wider active:bg-[#404040] transition-colors"
        >
          DELETE
        </button>
